...
if "versionCode 1150" not in build or "versionName '2.3.0-stage1150'" not in build:
    errors.append('Build: stage 1150 version metadata not set')
...
for token in ['everyHomeTabOpensUniqueFunctionalPageAndBackReturnsHome','eachRouteHasItsOwnRequiredFunctionalControl','keyControlsAreActuallyClickableNotDead','fingerprint:']:
    if token not in test: errors.append(f'Navigation Integrity Gate V2 missing: {token}')
...
print('Stage 1150: unique functional pages + Navigation Integrity Gate V2: PASS')
