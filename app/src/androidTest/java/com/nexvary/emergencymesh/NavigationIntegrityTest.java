...
public class NavigationIntegrityTest {
 ...
 @Test public void menuButtonIsAlive(){setLanguage("ar");try(ActivityScenario<MainActivity>s=ActivityScenario.launch(MainActivity.class)){onView(withTagValue(is((Object)"menu"))).perform(click());onView(withId(android.R.id.list)).check(matches(isDisplayed()));Espresso.pressBack();onView(withTagValue(is((Object)"page:home"))).check(matches(isDisplayed()));}}
